...
-- 检查是否已经存在API文档菜单
SELECT COUNT(*) INTO @exists FROM sys_menus WHERE menu_code = 'API_DOCS' AND delete_flag = 'N';
...
IF @exists = 0 THEN
    ...
    SELECT 'API文档菜单添加成功' AS result;
ELSE
    SELECT 'API文档菜单已存在' AS result;
END IF;