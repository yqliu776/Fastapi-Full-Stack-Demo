...
> 修复进度：**已修复 50 项** / 跳过 2 项 / 待修复 0 项（截至 2026-03-24）
...
### C-02 `role_repository.delete_role` 调用不存在的方法 ✅ 已修复
...
### C-03 关联表创建缺少必填审计字段导致数据库约束违反 ✅ 已修复
...
### C-05 Refresh Token 通过 URL Query 参数传递 ✅ 已修复
...
### C-06 Access Token 与 Refresh Token 可互换使用 ✅ 已修复
...
### M-01 前端 Axios 拦截器大量重复（~360 行重复代码） ✅ 已修复
...
**对比：**
- K,M`users_router.py` 使用 `ResponseModel.success(data=..., message=..., process_time=...)`
...
### L-08 `RateLimitConfigRequest` 等模型误用 `Query` 注解 ✅ 已修复
